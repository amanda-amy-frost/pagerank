...
def check_stability(old_nodes, new_nodes):
    ...
    # the first couple iterations of pagerank()
    if not old_nodes or not new_nodes:
        return False
...
def pagerank(dg):
    ...
    # i.e. same structure as <top_nodes> in random_surfer()
    old_top_nodes = []
...
def print_surfer_results(dg):
    """Prints formatted results for random_surfer(<dg>)"""
    ...
    start = time.time()
    total, normalized_total, top_nodes, top_nodes_normal = random_surfer(dg)
...
def print_pagerank_results(dg):
    """Prints formatted results for pagerank(<dg>)"""

    # constant used to help with formatting
    NODES_FORMAT_LENGTH = int(log10(len(dg.nodes))) + 1

    start = time.time()
    iterations, stability, total, top_nodes = pagerank(dg)
    elapsed = time.time() - start
...
if __name__ == '__main__':
    ...
    rank_top_nodes = print_pagerank_results(dg)
